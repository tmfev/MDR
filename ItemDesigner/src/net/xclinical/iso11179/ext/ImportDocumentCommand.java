...
public final class ImportDocumentCommand extends AbstractCommand {

	ReferenceDocument document;

	ReferenceDocument format;
	
	@Override
	public void invoke(TreeWriter writer, TreeSource source) throws RemoteException {
		document = TreeBinding.bind(source.subNode("document"));
		format = TreeBinding.bind(source.subNode("format"));

		if (document == null) throw new RemoteException("No document specified");
		if (format == null) throw new RemoteException("No format specified");
		
		FlatJsonExporter.copy(writer, Imports.doImports(document, format));		
	}
}
