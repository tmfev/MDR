/*
 * @(#)IfaceTracker.java	1.2  03/07/09
 * 
 ...
 */
class IfaceTracker extends ErrorTracker {

  /** Keep track of all objects of this type ever created. */
  static int typeCount = 0;
  static int innerCount = 0;

  IfaceTracker(String aName, ErrorTracker theOwner) {
    super(aName, "Interface");
    typeCount++;
    owner = theOwner;
    if (aName.indexOf('$') != -1) {
      // An inner interface
      ...
      if (theOwner.name.indexOf('$') != -1) {
         // This is an inner-inner interface
  ...
  void printErrorMessages(int level, HtmlTagWriter h) {
    ...
    if (error[level][NO_COMMENT]) {
        h.li();
        h.println( "Interface missing comment.");
        error[level][NO_COMMENT] = false;
    }
    if (error[level][NO_TEXT]) {
        h.li();
        h.println( "No text in interface comment.");
